...
app.post("/api/reservations", async (req, res) => {
    ...
    try {
        ...
        return res.json({
            message: "Votre réservation est bien enregistrée. Merci d'effectuer le virement avant le début du stage pour confirmer votre place.",
            iban: DEFAULT_IBAN,
...
app.post("/api/reservations/manage", async (req, res) => {
    ...
    try {
        ...
        const sessions = availabilityList
            ...
            .map((session) => {
            const reservedCount = reservations.filter((entry) => entry.sessionId === session.sessionId && activeStatuses.has(entry.status)).length;
            return {
